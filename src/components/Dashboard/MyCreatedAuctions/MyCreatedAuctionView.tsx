...
//         if (now >= startTime) {
//           console.log('[MyCreatedAuctionView] Auction start time reached, updating status');
//           loadAuctionDetails();
//           return;
//         }
//       }
...
//       if (auction.status === 'live' && auction.auctionDate && auction.auctionEndTime) {
//         const endTime = new Date(`${auction.auctionDate}T${auction.auctionEndTime}:00`);
//         if (now >= endTime) {
//           console.log('[MyCreatedAuctionView] Auction end time reached, updating status');
//           loadAuctionDetails();
//         }
//       }
...
//           console.log('[MyCreatedAuctionView] User details fetched:', result.user);
//         }
//       }
//     } catch (error) {
//       console.error('[MyCreatedAuctionView] Failed to fetch user details:', error);
//     }
//   };
...
//           console.log('[MyCreatedAuctionView] Live data updated - Bids:', data.auction.statistics?.total_bids || 0);
...
//       if (!preBidsLoading) {
//         const preBidsResponse = await fetch(`${API_BASE_URL}/auction/${id}/prebids`, {
//           method: 'GET',
//           headers: {
//             'Authorization': `Bearer ${token}`,
//             'Content-Type': 'application/json'
...
//           if (preBidsResult.success && preBidsResult.prebids && isMountedRef.current) {
...
//         const participantsResponse = await fetch(`${API_BASE_URL}/auction/${id}/participants`, {
//           method: 'GET',
//           headers: {
//             'Authorization': `Bearer ${token}`,
//             'Content-Type': 'application/json'
...
//           if (participantsResult.success && participantsResult.participants && isMountedRef.current) {
...
//       }
//     } catch (error) {
//       console.error('[MyCreatedAuctionView] Error fetching participants:', error);
//     } finally {
...
//           console.log('[MyCreatedAuctionView] Pre-bids fetched:', result.prebids);
//         }
//       } else {
//         console.warn('[MyCreatedAuctionView] Failed to fetch pre-bids:', response.status);
//       }
//     } catch (error) {
...
//   const handlePreBidAction = async (preBidId: string, action: 'approve' | 'reject') => {
//     try {
//       const token = localStorage.getItem("authToken") || localStorage.getItem("token") || localStorage.getItem("accessToken");
//       if (!token) return;

//       const response = await fetch(`${API_BASE_URL}/auction/prebid/${preBidId}/${action}`, {
//         method: 'POST',
//         headers: {
//           'Authorization': `Bearer ${token}`,
//           'Content-Type': 'application/json'
...
//       }
//     } catch (error) {
//       console.error(`[MyCreatedAuctionView] Error ${action}ing pre-bid:`, error);
//       toast.error(`Failed to ${action} pre-bid`);
//     }
//   };
...
//   participants: rawAuction.participants?.map((p: any) => p.phone_number) || [],
//   documents: rawAuction.documents?.map((d: any) => ({
//     name: d.file_name,
//     url: d.file_url,
//     size: d.file_type
//   })) || [],
...
//             console.log('[MyCreatedAuctionView] New API fetch successful:', auctionData);
//           }
//         }
//       } catch (newApiErr: any) {
//         console.warn('[MyCreatedAuctionView] New auction details API failed, trying legacy:', newApiErr);
//       }
...
//           console.log('[MyCreatedAuctionView] Legacy API fetch successful:', auctionData);
//         } catch (apiErr: any) {
...
//   const backendParticipants = allParticipants.length > 0 ? allParticipants : (backendMeta?.participantsList || []);
...
//         <div>
//           <h1 className="auction-title">My Created Auction: {auction.auctionNo}</h1>
...
//           Auction Management
//         </h2>
//         <div className="auction-management-actions">
//           {auction.status === 'live' && (
//             <button
//               type="button"
//               onClick={() => setLiveUpdatesEnabled(!liveUpdatesEnabled)}
//               className={`btn ${liveUpdatesEnabled ? 'btn-primary' : 'btn-secondary'}`}
//               title={liveUpdatesEnabled ? 'Pause live updates' : 'Resume live updates'}
//             >
//               {liveUpdatesEnabled ? '⏸️ Pause Updates' : '▶️ Resume Updates'}
//             </button>
//           )}
//           {auction.status === 'upcoming' && (
//             <>
//               <button type="button" onClick={handleEditAuction} className="btn btn-secondary">
//                 <Edit className="w-4 h-4" />
//                 Edit Auction
//               </button>
//               <button type="button" onClick={handleStartAuction} className="btn btn-primary">
...
//                   {auction?.status === 'live' && !isLiveUpdating && liveUpdatesEnabled && <span className="live-indicator">🟢</span>}
//                   {auction?.status === 'live' && !liveUpdatesEnabled && <span className="live-indicator">⏸️</span>}
//                 </label>
//                 <div className="info-value">
//                   {backendMeta.statistics.total_bids || 0}
//                   {lastUpdated && auction?.status === 'live' && (
//                     <small style={{ fontSize: '0.7rem', opacity: 0.7, marginLeft: '0.5rem' }}>
...
//               <div key={preBid.id || index} className="participant-item prebid-item">
//                 <div className="participant-info">
//                   <div className="participant-avatar">
//                     {(preBid.person_name || preBid.company_name || 'B').charAt(0).toUpperCase()}
//                   </div>
//                   <div className="participant-details">
//                     <h4>{preBid.person_name || preBid.company_name || 'Bidder'}</h4>
//                     <p>{preBid.company_name || preBid.phone_number || 'N/A'}</p>
//                     <p className="prebid-amount">
//                       <IndianRupee className="w-4 h-4" />
//                       {auction?.currency} {Number(preBid.amount || preBid.bid_amount).toLocaleString()}
//                     </p>
//                     <p className="prebid-time">
//                       {preBid.created_at || preBid.bid_time ?
//                         new Date(preBid.created_at || preBid.bid_time).toLocaleString() :
//                         'Unknown time'
//                       }
//                     </p>
//                   </div>
...
//                     preBid.status === 'approved' ? 'status-active' :
//                     preBid.status === 'rejected' ? 'status-inactive' :
//                     'status-pending'
//                   }`}>
...
//             Registered Participants
//             <span className="participants-count">
//               ({backendParticipants.length} total, {backendParticipants.filter((p: any) => p.status === 'joined').length} active)
//             </span>
//           </h2>
//           <button
//             type="button"
...
//                   {backendParticipants.filter((p: any) => p.status === 'joined').length}
//                 </span>
//               </div>
//               {/* <div className="summary-stat">
...
//                 <div className="participant-info">
//                   <div className="participant-avatar">
//                     {(participant.person_name || participant.phone_number || '?').charAt(0).toUpperCase()}
//                   </div>
//                   <div className="participant-details">
//                     <h4>{participant.person_name || 'N/A'}</h4>
//                     <p className="participant-company">
//                       {participant.company_name ? `${participant.company_name} • ` : ''}{participant.phone_number}
//                     </p>
//                     {participant.user_id && (
//                       <p className="participant-id">User ID: {participant.user_id}</p>
//                     )}
...
//                         <span className="timestamp">
//                           ✅ Joined: {new Date(participant.joined_at).toLocaleDateString()} {new Date(participant.joined_at).toLocaleTimeString()}
//                         </span>
//                       )}
//                     </div>
//                   </div>
...
//                   <div className={`participant-status status-${participant.status || 'invited'}`}>
        ...
        <div className="auction-management-actions">
          {auction.status === "live" && (
            <button
              ...
              onClick={() => setLiveUpdatesEnabled(!liveUpdatesEnabled)}
              className={`btn ${
                liveUpdatesEnabled ? "btn-primary" : "btn-secondary"
              }`}
              title={
                liveUpdatesEnabled
                  ? "Pause live updates"
