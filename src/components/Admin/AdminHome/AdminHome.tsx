...
// Updated interface to match the processed data from adminService.getDashboard()
interface AuctionItem {
  id: number;
  title: string;
  company: string;
  startTime: string; // This will now be the formatted string "DD/MM/YYYY HH:MM"
  decrementalValue: number; // Correctly typed as a number
  participants: number;
  auctionNo?: string;
}
...
const AdminHome: React.FC = () => {
  ...
  // Use the updated type for upcomingAuctions
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        // We are now using the enhanced getDashboard method from adminService
        const res = await adminService.getDashboard(token || undefined);

        if (!mounted) return;

        const { overview, recent_activities, upcoming_auctions } = res;
        ...
        setDashboardStats({
          ...
          todayAuctions: 0, // Not available in API
        ...
        // The adminService now handles the mapping and formatting
        setRecentActivities(recent_activities.slice(0, 10).map((activity: any) => ({
          id: activity.id,
        ...
        // upcoming_auctions is already processed by the service
        setUpcomingAuctions(upcoming_auctions);
