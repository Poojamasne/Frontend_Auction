...
// );
// const API_BASE_URL = (RUNTIME_BASE || `${BASE_URL}/auction`).replace(/\/?$/,'');
...
//   // Normalize various backend shapes to a flat array and total count
//   let list: any[] = [];
...
//     total = json.total ?? json.participants_total ?? json.participants.length;
//   }
//   // 2) { participants: { total, list: [...] } }
//   else if (json?.participants && Array.isArray(json.participants.list)) {
//     list = json.participants.list;
//     total = json.participants.total ?? list.length;
//   }
//   // 3) { data: [...] } or { list: [...] } or { users: [...] }
//   else if (Array.isArray(json?.data)) {
//     list = json.data;
//     total = json.total ?? json?.meta?.total ?? list.length;
...
//   // Fallback: nested { data: { participants: [...] }} or { data: { participants: { list }}}
//   else if (Array.isArray(json?.data?.participants)) {
//     list = json.data.participants;
//     total = json.data.total ?? list.length;
//   } else if (json?.data?.participants && Array.isArray(json.data.participants.list)) {
//     list = json.data.participants.list;
//     total = json.data.participants.total ?? list.length;
//   }

//   // Ensure array result
//   if (!Array.isArray(list)) list = [];
//   if (typeof total !== 'number') total = list.length;
//   return { total, participants: list };
// }
...
// const TIMERS_ENDPOINT = `${BASE_URL}/my-auctions/timers`;
...
export interface AuctionDetail extends BaseAuction {
  ...
  // Auctioneer details - inherited from BaseAuction but explicitly defined for clarity
  auctioneerCompany?: string;
  auctioneerPerson?: string;
  auctioneerEmail?: string;
  auctioneerAddress?: string;
