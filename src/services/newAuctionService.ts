...
const API_BASE_URL = `${BASE_URL}/auction`;
// const API_BASE_URL = `${BASE_URL}/auction/join`;
...
export interface CreateAuctionRequest {
  ...
  // participants may be an array of strings or a comma-separated string
...
export interface SubmitPreBidRequest {
  auction_id: number;
  phone_number: string;
  amount: number;
}
...
class AuctionService {
  ...
  // return this.handleRequest<AuctionResponse>(`/auction/${auctionId}/join-auctioneer`,

  // async joinAuction(data: JoinAuctionRequest): Promise<AuctionResponse> {
  //   const auctionId = data.auction_id;
  //   return this.handleRequest<AuctionResponse>(
  //     `/${auctionId}/join-auctioneer`,
  //     {
  //       method: "POST",
  //       headers: this.getAuthHeaders(true),
  //       body: JSON.stringify({
  ...
  // 📌 Submit Pre-Bid
  async submitPreBid(data: SubmitPreBidRequest): Promise<AuctionResponse> {
    if (!data.auction_id) throw new Error("Auction ID is required");
    if (!data.phone_number) throw new Error("Phone number is required");
    if (!data.amount || data.amount <= 0)
      throw new Error("Valid bid amount is required");

    return this.handleRequest<AuctionResponse>("/pre-bid", {
      method: "POST",
      headers: this.getAuthHeaders(true),
      body: JSON.stringify(data),
    });
  ...
  // 📌 Get My Pre-Bid for an auction
  async getMyPreBid(auctionId: number): Promise<any> {
    if (!auctionId) throw new Error("Auction ID is required");

    return this.handleRequest<any>(`/${auctionId}/my-prebid`, {
      method: "GET",
      headers: this.getAuthHeaders(true),
    });
  }
...
//   async joinAuction(
//     auctionId: number,
//     data: JoinAuctionRequest
//   ): Promise<AuctionResponse> {
//     return this.handleRequest<AuctionResponse>(
//       `/auction/${auctionId}/join-auctioneer`,
//       {
//         method: "POST",
//         headers: this.getAuthHeaders(true),
//         body: JSON.stringify(data),
...
// export interface JoinAuctioneerRequest {
//   auction_id: number;
// }
...
//   // 📌 Join as Auctioneer (based on your curl command)
//   async joinAsAuctioneer(auctionId: number): Promise<AuctionResponse> {
//     if (!auctionId) throw new Error("Auction ID is required");
