...
// const ADMIN_REPORTS_BASE = `${API_BASE_URL}/admin/reports`;
// // const ADMIN_REPORTS_BASE = `${API_BASE_URL}/admin/reports/Overview`;
...
//  GET /api/admin/reports/Overview
//  GET /api/admin/reports/report/auction-performance?filter={today|this_week|this_month|this_year}
//  GET /api/admin/reports/reports/user-activity?period={today|this_week|this_month|this_year}
...
export interface AdminOverviewReport {
  ...
  monthlyTrends?: Array<{month: string, auctions: number, revenue: number}>;
  [k: string]: any;
}
...
class AdminReportService {
  ...
  private extractArray(payload: any): any[] {
    ...
    if (!payload || typeof payload !== 'object') return [];
    // Common nested property names
    const candidateKeys = ['report', 'data', 'list', 'records', 'auctions', 'users', 'items', 'result', 'user_activity_report', 'trends', 'monthly_trends'];
    for (const key of candidateKeys) {
      if (Array.isArray((payload as any)[key])) return (payload as any)[key];
    }
    // Scan first array of objects in object values
  ...
  private normalizeOverview(raw: any): AdminOverviewReport {
    ...
    // Extract monthly trends if available
    if (raw.monthlyTrends || raw.monthly_trends || raw.monthlyData) {
      const trends = raw.monthlyTrends || raw.monthly_trends || raw.monthlyData;
      if (Array.isArray(trends)) {
        overview.monthlyTrends = trends.map((item: any) => ({
          month: item.month || item.month_name || '',
          auctions: Number(item.auctions || item.total_auctions || 0),
  ...
  async getOverview(token?: string): Promise<AdminOverviewReport> {
    ...
    if (!res.ok) throw new Error(json.message || `Failed to fetch overview (${res.status})`);
    const data = json.data || json.overview || json;
    return this.normalizeOverview(data);
  }
  ...
  async getMonthlyTrends(period: 'last_6_months' | 'last_12_months' = 'last_6_months', token?: string): Promise<MonthlyTrendItem[]> {
    try {
      // First try dedicated endpoint
      const url = `${ADMIN_REPORTS_BASE}/reports/monthly-trends?period=${encodeURIComponent(period)}`;
      const res = await fetch(url, { headers: this.getAuthHeaders(token) });
      ...
      if (res.ok) {
        ...
        let list = json.trends || json.data || json.monthly_trends || [];
        if (!Array.isArray(list)) list = this.extractArray(list);
        if (!Array.isArray(list)) return [];
      ...
      console.warn('Monthly trends endpoint not available, will use overview data if available');
    }
    
    // If dedicated endpoint fails, return empty array (will be filled from overview if available)
    return [];
  }
}
